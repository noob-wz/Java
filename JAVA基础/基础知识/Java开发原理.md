...
| **示例**         | `public` 转化成 UTF-8 编码的 $0$ 和 $1$ 序列。                                         | Java 源代码中的 `a = b + 1;` 被转化为 **Bytecode**（`.class` 文件）。                  |
...
2.  **解码：** 编译器首先用操作系统的文本编码（例如 `-encoding utf-8`）**读取并解码** `Hello.java` 文件。此时，编译器不再关心原始文件是用 UTF-8 还是 GBK 编码的，**解码后会将所有文本字符转化为统一的 Unicode 字符**（在 Java 内部，通常以 UTF-16 的形式存储）
3.  **编译转化：** 接着，编译器**检查代码的语法是否正确，并理解程序逻辑**，将 **程序逻辑** （如 `System.out.println(...)`）转化为 **Java 字节码 (Bytecode)(本质就是二进制）**
    - Java 字节码就是 JVM 能够理解和执行的、平台无关的二进制指令集
    - 这个步骤编译器（javac）直接将 Java 语句转化为 二进制的字节码指令，实际没有存储一个中间的文本指令文件。我们看到的`getstatic`、`ldc`以及.class 文件是通过工具反向翻译成人类可读的文本助记符，供调试分析
4.  **结果：** 生成 `.class` 文件。这个 `.class` 文件就是**程序执行的二进制**（Bytecode），它包含 Java 虚拟机指令集的指令码。
...
**当在命令行输入`javac Hello.java`，编译器会分析语法是否正确，并理解程序逻辑。接着会把程序转化成 JAVA 字节码，生成.class 文件（字节码文件）。命令行输入`java Hello`之后，JVM 会实时把字节码翻译成当前操作系统能够理解的机器码。**
