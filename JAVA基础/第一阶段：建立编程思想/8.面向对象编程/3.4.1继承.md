...
**重点说明**：**继承的本质不是为了把父类的代码拿过来（复用），而是为了让子类能够被当成父类（is-A)来使用（多态），具体说是“类型抽象+多态”**。

如果我就是**想借用一下别的类的功能，但我又不想成为它的子类”，该怎么办？** **答案是：组合**

这是架构设计中的黄金法则：**多用组合，少用继承**

**组合的目标**：不使用 `extends` 也能复用代码，而且更灵活。
