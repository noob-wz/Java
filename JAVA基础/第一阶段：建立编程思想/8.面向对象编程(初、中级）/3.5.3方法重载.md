如果说 **“方法重写 (Override)”** 是 JVM 在运行时的 **“动态智慧”**，那么 **“方法重载 (Overload)”** 就是编译器在编译时的 **“静态硬编码”**。
...
**核心定义**： 在同一个类中，方法名相同，但 **参数列表不同**（类型、个数、顺序）的一组方法。     
**一句话本质**：**“同名不同人”**。对开发者来说名字一样，但 **在 JVM 眼里，它们是完全不同的两个方法，没有任何关系。**
...
**编译后的字节码 (javap -s)**： 虽然名字都叫 `test`，但在常量池中，它们是截然不同的符号引用：
...
**结论**：**重载是“编译期多态”**。在编译生成 `.class` 文件的那一刻，具体调用哪个方法 **已经被写死** 了。根本不需要像“重写”那样在运行时查虚方法表。
