...
**一句话总结：** 当方法中不涉及任何和对象相关的成员，就应该将方法设计成静态方法，提高开发效率
...
**口诀：** 静态方法只能访问静态成员
**根本原因（生命周期维度）**：**静态内容在类加载阶段就已经创建，而非静态内容需要创建对象后才会存在。在类加载初期，无法找到静态成员访问。故而 `this` 和 `super` 等
关键字在静态方法中使用会报错。报错原因就是因为静态方法和非静态方法方法的调用逻辑不同，后者会在编译期就会隐式传入 `this`。**
...
**✅ 修复方案**：
* **方案一：** `appName` 也变成 `static`
* **方案二：** 创建对象去访问
* **方案三：** 在静态方法里把对象显式传进去。
  * 既然 JVM 不会自动传 `this`，那就手动传一个对象参数。
  ...
  public class User {
    ...
    // 静态方法：工具人
    public static void sayHi(User u) { // <--- 手动把对象传进来
        // ✅ 现在可以访问了，因为你指定了是 "这个u" 的 name
...
**“静态方法继承”**，这个词在 Java 里有点误导性，其实是 **“伪继承”**。

* **真正的继承**：子类继承了父类的“虚方法表”，如果子类不重写，vtable 里那个方法的地址就指向父类的方法地址。
* **静态方法的继承**：其实是 **编译器解析规则** 的一种便利。
  * 当你写 `Cat.test()` 时，编译器会去 `Cat` 类里找 `test`。
  * 如果 `Cat` 自己定义了（遮蔽），编译器就生成 `invokestatic Cat.test`。
  * 如果 `Cat` 没有定义，编译器会沿着继承链往上找，发现 `Animal` 里有，编译器就生成 `invokestatic Animal.test`。
