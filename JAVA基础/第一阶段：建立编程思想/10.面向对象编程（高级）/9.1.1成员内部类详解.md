...
成员内部类访问外部类直接通过变量名访问即可，也可以通过 `OuterClassName.this.变量名` 访问。而外部类访问成员内部类需要创建实例来访问，即 `实例名.变量名` 访问
...
反向访问同样需要合成方法。当外部类访问内部类的私有成员时，编译器在内部类中生成访问方法。也说明，外部类也具有访问内部类实例的私有成员特权
